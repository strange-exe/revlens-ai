  ...
  return (
    <div className="flex min-h-screen bg-(--color-surface) dark:bg-(--color-surface-dark) transition-colors duration-300">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 border-b border-(--color-border)/60 dark:border-(--color-border-dark)/60 flex items-center justify-between px-4 sm:px-6 bg-(--color-surface-elevated)/80 dark:bg-(--color-surface-elevated-dark)/80 backdrop-blur-xl shadow-sm dark:shadow-black/5">
          <div className="flex items-center gap-3">
            <button className="md:hidden p-1.5 rounded-lg hover:bg-(--color-brand-50) dark:hover:bg-(--color-brand-800) text-(--color-muted) dark:text-(--color-muted-dark) cursor-pointer">
              <Menu size={18} />
            </button>
            <h2 className="font-heading text-base font-bold text-(--color-brand-600) dark:text-white">Dashboard</h2>
          </div>
