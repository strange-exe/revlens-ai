  ...
  return (
    <section className="relative min-h-[92vh] flex items-center overflow-hidden bg-gradient-to-b from-(--color-brand-50) to-(--color-surface) dark:from-[#091510] dark:to-(--color-surface-dark)">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute -bottom-40 -left-40 w-[500px] h-[500px] rounded-full bg-(--color-gold-100)/20 dark:bg-(--color-gold-900)/20 blur-[100px]" />
        ...
        <svg className="absolute top-48 left-[6%] w-8 h-8 text-(--color-gold-300)/20 dark:text-(--color-gold-700)/20 animate-float" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1">
        ...
        <div className="absolute bottom-[30%] right-[30%] w-px h-28 bg-gradient-to-t from-(--color-gold-300)/15 to-transparent dark:from-(--color-gold-700)/15" />
      </div>
      ...
      <div className="relative w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 sm:py-32 lg:py-36">
        <div className="flex flex-col lg:flex-row lg:items-center lg:gap-16">
          ...
          <div className="flex-1 max-w-2xl">
            ...
            <div className="animate-slide-up mt-10 flex flex-wrap gap-3 delay-3">
              <Link
                to={ctaLink}
                className="group relative inline-flex items-center gap-2.5 px-7 py-3.5 rounded-xl bg-(--color-brand-600) dark:bg-(--color-brand-500) text-white text-sm font-bold overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-0.5"
              >
              ...
              <a
                ...
                className="inline-flex items-center gap-2.5 px-7 py-3.5 rounded-xl glass text-sm font-medium text-(--color-muted) dark:text-(--color-muted-dark) hover:text-(--color-brand-500) dark:hover:text-white transition-all duration-300 hover:-translate-y-0.5"
              >
          ...
          <div className="hidden lg:block flex-1 max-w-md animate-fade-in delay-4">
            <div className="relative">
              {/* Glow behind card */}
              <div className="absolute -inset-4 rounded-3xl bg-gradient-to-br from-(--color-brand-400)/20 via-(--color-gold-300)/10 to-(--color-brand-600)/20 blur-2xl animate-pulse-soft" />
              ...
              <div className="relative rounded-2xl overflow-hidden border border-(--color-border) dark:border-(--color-border-dark) bg-white/90 dark:bg-(--color-surface-elevated-dark)/90 backdrop-blur-xl shadow-2xl">
                ...
                <div className="flex items-center gap-2 px-5 py-3 border-b border-(--color-border)/60 dark:border-(--color-border-dark)/60">
                  <div className="flex gap-1.5">
                    ...
                    <div className="w-2.5 h-2.5 rounded-full bg-(--color-gold-400)/80" />
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      { icon: <Star size={14} />, label: "Rating", value: "4.8", color: "text-(--color-gold-500)" },
                  ...
                  <div className="rounded-xl bg-(--color-surface-muted) dark:bg-(--color-surface-muted-dark) p-4">
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        { label: "Neutral", w: "15%", color: "bg-(--color-gold-400)" },
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-0.5 mt-1">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <Star key={i} size={9} className="fill-(--color-gold-400) text-(--color-gold-400)" />
                        ))}
                      </div>
                    </div>
                  </div>
