  ...
  return (
    <div ref={containerRef}>
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-(--color-brand-50) to-(--color-surface) dark:from-[#091510] dark:to-(--color-surface-dark) pt-32 pb-20">
        ...
        <div className="absolute -bottom-32 -left-32 w-[500px] h-[500px] rounded-full bg-(--color-gold-100)/20 dark:bg-(--color-gold-900)/20 blur-[100px]" />
        ...
        <div className="relative max-w-3xl mx-auto px-4 sm:px-6">
          ...
          <div className="reveal gold-divider mt-6 mb-8" />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {values.map((v, i) => (
              <div
                key={v.title}
                className="reveal group rounded-2xl widget-card p-7 hover:-translate-y-0.5"
              ...
              >
                ...
                <p className="mt-2 text-sm text-(--color-muted) dark:text-(--color-muted-dark) leading-relaxed">{v.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative py-20 lg:py-28 overflow-hidden bg-(--color-surface-muted) dark:bg-(--color-surface-muted-dark)">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div className="reveal rounded-2xl widget-card p-7">
            ...
            <div className="reveal rounded-2xl widget-card p-7">
              <div className="flex items-center gap-2.5 mb-5">
                <div className="p-2 rounded-lg bg-(--color-gold-100) dark:bg-(--color-gold-900)/40">
                  <Rocket size={16} className="text-(--color-gold-500) dark:text-(--color-gold-400)" />
                </div>
              ...
              <ul className="space-y-4">
                {[
                  ...
                  <li key={item} className="flex items-center gap-3 text-sm">
                    <span className="w-1.5 h-1.5 rounded-full bg-(--color-gold-400) shrink-0" />
              ...
              {timeline.map((t, i) => (
                <div key={t.phase} className="reveal relative pl-12 sm:pl-14" style={{ transitionDelay: `${i * 0.08}s` }}>
                  ...
                  <div className={`absolute left-2.5 sm:left-3.5 top-1 w-3 h-3 rounded-full border-2 transition-all
                    ${t.status === "done"
                      ? "bg-(--color-brand-400) border-(--color-brand-400) shadow-[0_0_8px_rgba(61,158,111,0.4)]"
                      : t.status === "current"
                        ? "bg-(--color-gold-400) border-(--color-gold-400) shadow-[0_0_8px_rgba(212,167,44,0.4)] animate-pulse-soft"
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${
                        t.status === "done"
                          ...
                          : t.status === "current"
                            ? "bg-(--color-gold-100) text-(--color-gold-600) dark:bg-(--color-gold-900)/40 dark:text-(--color-gold-400)"
                            : "bg-(--color-surface-muted) text-(--color-muted) dark:bg-(--color-surface-muted-dark) dark:text-(--color-muted-dark)"
                      }`}>
                        {t.phase}
                      </span>
                      {t.status === "current" && (
                        <span className="text-[10px] font-bold text-(--color-gold-500) dark:text-(--color-gold-400) uppercase tracking-wider">In Progress</span>
                      )}
                      {t.status === "done" && (
        ...
        <div className="relative max-w-3xl mx-auto px-4 sm:px-6 py-20 lg:py-24 text-center">
          ...
          <div className="reveal mt-8 flex flex-wrap justify-center gap-3">
            <a
              ...
              className="group inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white text-(--color-brand-600) text-sm font-bold hover:bg-(--color-gold-50) transition-all shadow-xl hover:shadow-2xl hover:-translate-y-0.5"
            >
            ...
            <Link
              to="/dashboard"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-white/20 text-white text-sm font-semibold hover:bg-white/10 transition-all"
            >
              Try Dashboard
            </Link>
          </div>
